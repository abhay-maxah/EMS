...
import ApplyView from '../pages/ApplyLeaveForm'; // ✅ Added import
...
const Clock = () => {
  ...
  const [noteInput, setNoteInput] = useState('');
  const [lastWorkDuration, setLastWorkDuration] = useState(null);
  const [showApplyLeave, setShowApplyLeave] = useState(false); // ✅ Modal for Apply Leave
  ...
  return (
    <div className="bg-white rounded-3xl shadow-2xl p-10 max-w-3xl mx-auto space-y-10">
      {/* Header */}
      <div className="text-center space-y-2">
        <h1 className="text-3xl font-bold text-blue-900">{company?.name || 'Company Name'}</h1>
      </div>
      ...
      {!isClockedIn && lastWorkDuration && (
        <div className="text-center text-lg text-gray-700">
          ✅ You worked <span className="text-blue-600 font-bold">{lastWorkDuration}</span> today. Great job!
        </div>
      )}
        ...
        {!isClockedIn ? (
          ...
          <>
            {!isOnBreak ? (
              <button
                onClick={handleTakeBreak}
                  className="bg-blue-900 hover:bg-blue-800 text-white px-6 py-3 rounded-xl shadow-md flex items-center gap-2"
              >
                  <FaPause /> Take a Break
              </button>
            ) : (
      ...
      {showApplyLeave && (
        <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50">
          <div className="bg-white rounded-2xl p-6 w-[90%] max-w-4xl shadow-2xl overflow-y-auto max-h-[90vh]">
...
const DigitSlide = ({ value, isStatic = false }) => {
  ...
  if (isStatic) {
    return (
      <div className="w-[40px] h-[64px] flex items-center justify-center text-5xl font-mono text-blue-900">
        {value}
      </div>
    );
  }
  ...
  return (
    <div className="w-[40px] h-[64px] relative" style={{ perspective: '800px' }}>
      <div
        className="absolute w-full h-full flex items-center justify-center text-5xl font-mono text-blue-900 bg-white"
        style={{
